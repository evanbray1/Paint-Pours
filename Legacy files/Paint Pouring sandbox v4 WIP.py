...
num_images = 50                      #How many images do you want to produce?
make_surface_plot = False           #Helpful for diagnostic purposes in case you want to see a low-res surface plot of your image

custom_cmap = 'any'                 #Which colormap do you want to use for your images? Use "any" to pick one at random. Or pick one from this list: https://matplotlib.org/stable/tutorials/colors/colormaps.html
output_directory = 'to_print/'   #The relative directory where the output images will be saved
########################################
...
for i in range(num_images):
    ...
    if custom_cmap not in bad_cmaps:
        while any(cmap in s for s in bad_cmaps):
            print('Randomly chose an ugly colormap! Choosing again...')
            cmap = np.random.choice(non_reversed_colormaps)
            
    cmap = plt.cm.get_cmap(cmap)    #Retrieve the colormap
    cmap_name = cmap.name           #What is the name of the chosen colormap?
    ...
    if save_image == True:
        if custom_cmap == 'any':    #Change the filename depending on whether you opted to use a random colormap or not.
            filename = cmap_name+'_'+str(num_levels)+'levels_'+'_'.join(['{:.2f}'.format(i) for i in octave_powers[1:]])+'_stretch'+str(stretch_value)+\
                '.png'
        else:
        ...
        output_directory_temp = output_directory#+cmap_name+'/'
