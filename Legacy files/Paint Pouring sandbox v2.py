...
v_mag = np.zeros(droplets_per_point)+0.5
...
@njit    
def particle_forces2(coords,npoints,particle,charge):
    ...
    F_from_others_x = -charge*dx_from_others/r_from_others**2
    F_from_others_y = -charge*dy_from_others/r_from_others**2
    result = np.array([np.sum(F_from_others_x),np.sum(F_from_others_y)])
    return result

@njit
def update_velocities(v_x_temp,v_y_temp,F_total,mass,dt):
    v_x_temp += F_total[0]/mass*dt
    v_y_temp += F_total[1]/mass*dt
    return v_x_temp,v_y_temp

def animate(i,coords,v_x,v_y,v_mag,dt,charge,test_counter):
    print('======',i)
    # print('counter is ',test_counter)
    # test_counter += 1
    print(v_x)
    # print(v_mag)
    # os.sys.exit()
    ...
    for particle in range(0,num_particles_temp):
        interparticle_forces = particle_forces2(coords,num_particles_temp ,particle,charge)    
        F_total_x[particle],F_total_y[particle] = interparticle_forces
    
    #Subtract off the force of static friction from each particle
    # for particle in range(0,num_particles_temp):
    F_friction_x = static_friction_force*np.sign(v_x)*v_x/v_mag
    # F_total_x -= F_friction_x
    print('F_friction_x is ',F_friction_x[0])
    
    # F_total_y -= static_friction_force*np.sign(v_y[particle])*v_y[particle]/v_mag[particle]
    # print(len(v_x))
    # print(len(v_mag))
    ...
    # print('Current v is ',v_x[0],v_y[0])
    # print('Current F is ',F_total[:,0])
    v_x,v_y = update_velocities(v_x,v_y,F_total,1,dt)
    # print(len(v_x))
    
    
    # print('Force_x on 0 is: ',F_total_x[0])
    # print('velocity_x of 0 is: ',v_x[0])
    print('Current v_mag is ',v_mag[0])

    v_mag = np.sqrt(v_x**2+v_y**2)
    speeding_particles = np.where(abs(v_mag) > v_max)[0]
    if len(speeding_particles) > 0:
        v_x[speeding_particles] = v_max*v_x/v_mag
        v_y[speeding_particles] = v_max*v_y/v_mag
        v_mag[speeding_particles] = np.sqrt(v_x**2+v_y**2)
    print('Current v is ',v_x[0],v_y[0])
    print('Current v_mag is ',v_mag[0])
...
anim = animation.FuncAnimation(fig, animate,repeat=False,frames=num_frames, interval=16.6667*10, blit=False,fargs=(coords,v_x,v_y,v_mag,dt,charge,test_counter,))
