...
class PaintPour:
    ...
    def generate(self):
        ...
        if self.add_cells and not self.prominent_cells:
            warnings.warn(
                'WARNING: the add_cells parameter isnt fully implemented yet. Try setting prominent_cells=True instead for now')
        if self.add_cells:
            include_perimeter_regions = np.random.choice([True, False])
            self.gauss_smoothing_sigma = 6
            self.threshold_percentile = 70
            num_voronoi_points = int(np.random.choice(
                [100, 250, 600])*self.image_dimensions[0]*self.image_dimensions[1]/(1920*1080))
            ...
            if self.prominent_cells:
                ...
                self.paint_pour_surface = (self.paint_pour_surface - np.nanmin(self.paint_pour_surface)) / (
                    np.nanmax(self.paint_pour_surface) - np.nanmin(self.paint_pour_surface))
            else:
                self.paint_pour_surface[ind] = 1.01
                self.final_colormap.set_over(
                    self.base_colormap(np.random.uniform(0, 1)))
        ...
        if self.display_final_image:
            fig, ax = plt.subplots(1, figsize=(
                self.image_dimensions[0] / 120, self.image_dimensions[1] / 120))
            ax = plt.Axes(fig, [0., 0., 1., 1.])
            fig.add_axes(ax)
            ax.imshow(self.paint_pour_surface, cmap=self.final_colormap,
        ...
        if self.save_image:
            ...
            fig.savefig(os.path.join(
                self.output_directory, self.filename), dpi=120)
    ...
    def pick_paint_pour_colormap(self):
        ...
        if self.cmap_name == 'custom':
            if self.custom_cmap_colors is None:
                self.custom_cmap_colors = [
                    '#33192F', '#803D75', '#CF2808', '#FEE16E', '#6AA886', '#5CE5FB', '#1A1941']
            colors = self.custom_cmap_colors.copy()
            self.base_colormap = make_custom_colormap(
                colors=colors, show_plot=False)
        else:
            self.base_colormap = plt.cm.get_cmap(self.cmap_name)
        if self.show_intermediate_plots:
            plot_colormap(self.base_colormap,
                          title='Your base colormap, ' + self.base_colormap.name)
        if self.num_colormap_levels == 'continuous':
            self.final_colormap = self.base_colormap.copy()
        else:
            colors = np.random.randint(
                low=0, high=256, size=self.num_colormap_levels)
            nodes = np.sort(np.random.uniform(
                low=0, high=1, size=len(colors) - 1))
            self.final_colormap = make_custom_segmented_colormap(colors=self.base_colormap(colors), nodes=[
                                                                 0] + list(nodes) + [1], show_plot=self.show_intermediate_plots, cmap_name=self.base_colormap.name)
    ...
    def assign_unspecified_parameters(self):
        ...
        if self.octave_powers is None:
            ...
            # TODO, in the metadata .csv, why do these values get saved as text strings listing the datatype instead of raw values?
    ...
    def save_metadata_to_csv(self, filename):
        ...
        for attr in dir(self):
            # Ignore the methods and Python's built-in attributes
            if attr.startswith('__') or callable(getattr(self, attr)):
                continue

            value = getattr(self, attr)  # Grab the attribute
            # Make sure it's not an image or image cube (we don't want to save that here)
            if is_2d_or_3d_array(value):
                continue
...
def perlin_field(image_dimensions, octave, stretch, make_tileable=False, show_plots=False):
    ...
    if octave == 0:  # If we're doing the 0th octave, the vectors get positioned OUTSIDE the corners of the image. Since the 1st octave still produces too hilly of a surface unto itself.
        # X coordinates of vertices (Note the units here are not in pixels, but in "# of vertices from the left". I call this the "cell coordinate system").
        vector_coords_x = np.linspace(
            -0.5*image_dimensions[0], 1.5*image_dimensions[0], grid_dimensions[0], dtype=np.float32)
        # Y coordinates of vertices (Note the units here are not in pixels, but in "# of vertices from the bottom")
        vector_coords_y = np.linspace(
            -0.5*image_dimensions[1], 1.5*image_dimensions[1], grid_dimensions[0], dtype=np.float32)
        ...
        # X coordinates of vertices (Note the units here are not in pixels, but in "# of vertices from the left". I call this the "cell coordinate system").
        vector_coords_x = np.linspace(
            0, image_dimensions[0], grid_dimensions[0], dtype=np.float32)
        # Y coordinates of vertices (Note the units here are not in pixels, but in "# of vertices from the bottom")
        vector_coords_y = np.linspace(
            0, image_dimensions[1], grid_dimensions[1], dtype=np.float32)

    # What DIRECTION is each vector pointing?
    # What direction is the vector pointing at this vertex? (x-coordinate)
    vector_dir_x = (np.random.random(
        (grid_dimensions[1], grid_dimensions[0]))*2-1).astype('float32')
    # What direction is the vector pointing at this vertex? (y-coordinate)
    vector_dir_y = (np.random.random(
        (grid_dimensions[1], grid_dimensions[0]))*2-1).astype('float32')
    ...
    if octave == 0:  # Because I defined the 0th octave differently than the rest, the coordinates are defined slightly differently in this case.
        grid_points_x = np.linspace(
            0, grid_dimensions[0]-1, image_dimensions[0]*2, endpoint=False)
        grid_points_y = np.linspace(
            0, grid_dimensions[1]-1, image_dimensions[1]*2, endpoint=False)

        # Define some empty arrays that we'll store stuff in, shortly.
        # Where we'll store the results of the dot products of each grid point with each of the four nearest grid nodes.
        dot_products = np.zeros(
            (image_dimensions[1]*2, image_dimensions[0]*2, 4)).astype('float32')
        # Where we'll store the weighting values that we use for interpolating results of the [4]x[Image_width]x[Image_height] dot product array
        weights = np.zeros(
            (image_dimensions[1]*2, image_dimensions[0]*2, 4)).astype('float32')
        # Where we'll store the final image, which we fill in one "cell" at a time.
        image = np.zeros(
            (image_dimensions[1]*2, image_dimensions[0]*2)).astype('float32')

    else:
        grid_points_x = np.linspace(
            0, grid_dimensions[0]-1, image_dimensions[0], endpoint=False)
        grid_points_y = np.linspace(
            0, grid_dimensions[1]-1, image_dimensions[1], endpoint=False)

        # Define some empty arrays that we'll store stuff in, shortly.
        # Where we'll store the results of the dot products of each grid point with each of the four nearest grid nodes.
        dot_products = np.zeros(
            (image_dimensions[1], image_dimensions[0], 4)).astype('float32')
        # Where we'll store the weighting values that we use for interpolating results of the [4]x[Image_width]x[Image_height] dot product array
        weights = np.zeros(
            (image_dimensions[1], image_dimensions[0], 4)).astype('float32')
        # Where we'll store the final image, which we fill in one "cell" at a time.
        image = np.zeros(
            (image_dimensions[1], image_dimensions[0])).astype('float32')
    ...
    for i in range(num_cells_y):
        for j in range(num_cells_x):
            ...
            # This is an array of grid points with coordinates that fall in cell (i,j). Think of these coordinate points as "position within the current cell". These coordinate values range from [0,1)
            cell_coords_x = grid_points_x[np.where(
                (grid_points_x >= j) & (grid_points_x < (j+1)))]-j
            cell_coords_y = grid_points_y[np.where(
                (grid_points_y >= i) & (grid_points_y < (i+1)))]-i

            # turn those cell coordinates into a meshgrid of points.
            a, b = np.meshgrid(cell_coords_x, cell_coords_y)
            # Reshape those big 'ol 2D arrays into something that's easier for humans to work with.
            coordinates = np.array([a.ravel(), b.ravel()])

            # Count some things
            # number of unique points there are in this cell total
            num_points_in_cell = len(coordinates[0])
            # Number of unique x-coordinates there are for this cell
            num_points_in_cell_x = len(cell_coords_x)
            # Number of unique y-coordinates there are for this cell
            num_points_in_cell_y = len(cell_coords_y)
            ...
            # Generate a 2xN array where the corner vector's components are repeated N times. This is done so we can calculate all the grid points in this cell simultaneously.
            vector_temp = np.array([np.repeat(vector_dir_x[i, j], num_points_in_cell), np.repeat(
                vector_dir_y[i, j], num_points_in_cell)])
            # Perform the dot product of each individual grid point in that cell with that gradient vector.
            result_temp = np.reshape(np.sum(
                coordinates*vector_temp, axis=0), (num_points_in_cell_y, num_points_in_cell_x))
            # write the result to the appropriate region of the dot product result array
            dot_products[y_low:y_high, x_low:x_high, 2] = result_temp
            # Calculate the weight of each offset vector proximity from the bottom left corner vector
            weights[y_low:y_high, x_low:x_high, 2] = np.reshape(smootherstep_function(
                1-coordinates[0])*smootherstep_function(1-coordinates[1]), (num_points_in_cell_y, num_points_in_cell_x))

            # Then bottom right
            # recalculate the relative position of each point in the cell based on the bottom right corner
            coordinates_temp = np.array([coordinates[0]-1, coordinates[1]])
            vector_temp = np.array([np.repeat(vector_dir_x[i, j+1], num_points_in_cell),
                                   np.repeat(vector_dir_y[i, j+1], num_points_in_cell)])
            result_temp = np.reshape(np.sum(
                coordinates_temp*vector_temp, axis=0), (num_points_in_cell_y, num_points_in_cell_x))
            dot_products[y_low:y_high, x_low:x_high, 3] = result_temp
            weights[y_low:y_high, x_low:x_high, 3] = np.reshape(smootherstep_function(
                coordinates[0])*smootherstep_function(1-coordinates[1]), (num_points_in_cell_y, num_points_in_cell_x))

            # Then top right
            coordinates_temp = np.array([coordinates[0]-1, coordinates[1]-1])
            vector_temp = np.array([np.repeat(vector_dir_x[i+1, j+1], num_points_in_cell),
                                   np.repeat(vector_dir_y[i+1, j+1], num_points_in_cell)])
            result_temp = np.reshape(np.sum(
                coordinates_temp*vector_temp, axis=0), (num_points_in_cell_y, num_points_in_cell_x))
            dot_products[y_low:y_high, x_low:x_high, 0] = result_temp
            weights[y_low:y_high, x_low:x_high, 0] = np.reshape(smootherstep_function(
                coordinates[0])*smootherstep_function(coordinates[1]), (num_points_in_cell_y, num_points_in_cell_x))

            # Then top left
            coordinates_temp = np.array([coordinates[0], coordinates[1]-1])
            vector_temp = np.array([np.repeat(vector_dir_x[i+1, j], num_points_in_cell),
                                   np.repeat(vector_dir_y[i+1, j], num_points_in_cell)])
            result_temp = np.reshape(np.sum(
                coordinates_temp*vector_temp, axis=0), (num_points_in_cell_y, num_points_in_cell_x))
            dot_products[y_low:y_high, x_low:x_high, 1] = result_temp
            weights[y_low:y_high, x_low:x_high, 1] = np.reshape(smootherstep_function(
                1-coordinates[0])*smootherstep_function(coordinates[1]), (num_points_in_cell_y, num_points_in_cell_x))
    ...
    image = weights[:, :, 0]*dot_products[:, :, 0]+weights[:, :, 1]*dot_products[:, :,
                                                                                 1]+weights[:, :, 2]*dot_products[:, :, 2]+weights[:, :, 3]*dot_products[:, :, 3]
    ...
    if octave == 0:
        image = image[int(image_dimensions[1]/2):int(image_dimensions[1]+image_dimensions[1]/2),
                      int(image_dimensions[0]/2):int(image_dimensions[0]+image_dimensions[0]/2)]
    ...
    vector_info = [vector_coords_x,
                   vector_coords_y, vector_dir_x, vector_dir_y]
...
def fractal_noise(image_dimensions, relative_powers, stretch, show_perlin_noise_plots=False,
                  show_fractal_noise_plot=False, return_octaves=False):
    """
    Generate a fractal noise image by summing multiple Perlin noise octaves.

    This function works by calling the Perlin_field function multiple times, each time with a different octave,
    and adding the result to a master image array. Typically each octave gets multiplied by 1/(2^octave) before adding it to the master image array.
    ...
    for i in range(num_octaves):
        ...
        if relative_powers[i] > 0:
            perlin_image, vectors = perlin_field(
                image_dimensions, i, stretch, show_plots=show_perlin_noise_plots)
...
def _log_rescale_helper(input_values, exponent):
    ...
    """
    if exponent != 1:
        rescaled_values = np.log10(
            exponent * input_values + 1) / np.log10(exponent)
...
def log_rescaler(input_values, exponent, show_plot=False):
    ...
    # No point in showing this plot if the exponent is 1 since it will just be a straight line
    if (show_plot is True) and (exponent != 1):
...
def remove_perimeter_regions(thresholded_image):
    ...
    image_with_border = cv2.copyMakeBorder(
        thresholded_image, 1, 1, 1, 1, cv2.BORDER_CONSTANT, value=1)
...
def get_contour_pixel_areas(image, list_of_contours):
    ...
    for contour in list_of_contours:
        ...
        temp_image = cv2.drawContours(
            temp_image, [contour], contourIdx=-1, color=1, thickness=-1)
...
def remove_small_regions(image, size_threshold):
    ...
    """
    print('...Removing cells smaller than ' +
          str(int(size_threshold))+' pixels in area')
    contours, hierarchy = cv2.findContours(
        image, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    # Calculate the area in pixels of each contour in the image
    contour_areas = get_contour_pixel_areas(image, contours)
    # Identify the contours with an area smaller than the threshold
    ...
    for i, region in enumerate(small_regions):
        temp_image = cv2.drawContours(
            temp_image, contours, contourIdx=region, color=1, thickness=-1)
...
def pick_random_colormap(print_choice=False, show_plot=False):
    ...
    bad_cmaps = ['flag', 'Accent', 'gist_stern', 'Paired', 'Dark2', 'Set1', 'Set2', 'Set3', 'tab10', 'tab20', 'tab20c', 'tab20b', 'binary', 'Pastel1',
                 'Pastel2', 'gist_yarg', 'gist_gray', 'brg', 'CMRmap', 'gist_ncar', 'gist_rainbow', 'hsv', 'terrain', 'gnuplot2', 'nipy_spectral', 'prism']
    # Generate a list of all colormaps that don't contain "_r" in their name, indicating they are just a reversed version of another colormap. "Grays" and "Grays_r" look fundamentally the same for this type of art.
    ...
    if show_plot is True:
        plot_colormap(
            cmap, title=f'Your randomly-chosen base colormap, {cmap.name}')
    return cmap
...
def plot_colormap(cmap, title='Your colormap'):
    ...
    ax_cmap.imshow(np.outer(np.ones(100), np.arange(0, 1, 0.001)),
                   cmap=cmap, origin='lower', extent=[0, 1, 0, 0.1])
...
def make_custom_colormap(colors=None, nodes=None, show_plot=False):
    ...
    if colors is None:
        ...
        colors = ['#33192F', '#803D75', '#CF2808',
                  '#FEE16E', '#6AA886', '#5CE5FB', '#1A1941']
    if nodes is None:
        print('WARNING: No input nodes specified. Picking an evenly-spaced array....')
        nodes = np.linspace(0, 1.0, len(colors))
    cmap_custom = LinearSegmentedColormap.from_list(
        'custom', list(zip(nodes, colors)))

    if show_plot is True:
        fig_cmap, ax_cmap = plt.subplots(figsize=(12, 2))
        ax_cmap.imshow(np.outer(np.ones(100), np.arange(0, 1, 0.001)),
                       cmap=cmap_custom, origin='lower', extent=[0, 1, 0, 0.1])
        ax_cmap.set_title('Your custom colormap')
        ax_cmap.set_xticks(ticks=np.array(nodes))
        ax_cmap.set_xticklabels(['{:.2f}'.format(node)
                                for node in nodes], rotation=60)
        ax_cmap.set_yticks([])
        fig_cmap.tight_layout()
    return cmap_custom
...
def make_custom_segmented_colormap(colors=None, nodes=None, show_plot=False, cmap_name='custom'):
    ...
    cmap_custom = LinearSegmentedColormap.from_list(
        'custom', list(zip(nodes_new, colors_new)))
...
def make_cell_image(image_dimensions, num_voronoi_points=400, gauss_smoothing_sigma=6, threshold_percentile=70, minimum_cell_area=25,
    ...
    good_indices = np.where((x_new > 0) & (x_new < image_dimensions[0]) & (
        y_new > 0) & (y_new < image_dimensions[1]))[0]
    ...
    image = np.histogram2d(x_new, y_new, bins=image_dimensions, range=[
                           [0, image_dimensions[0]], [0, image_dimensions[1]]])[0].T
    if show_plots is True:
    ...
    image_threshold[image_proc < np.percentile(
        image_proc, threshold_percentile)] = 1
    if show_plots is True:
        ax2[0, 2].imshow(image_threshold, origin='lower')
        ax2[0, 2].set_title('Threshold applied')
        ax2[0, 2].set_aspect('equal')

    if include_perimeter_regions is False:
    ...
    image_final = (image_final - np.min(image_final)) / \
        (np.max(image_final) - np.min(image_final))
...
def calculate_cell_centroids(thresholded_cell_image):
    ...
    x, y = [np.arange(thresholded_cell_image.shape[1]),
            np.arange(thresholded_cell_image.shape[0])]
    x, y = np.meshgrid(x, y)

    # Determine the (x,y) of the various closed contours in a thresholded_cell_image
    list_of_contours, hierarchy = cv2.findContours(
        thresholded_cell_image, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)[1:]
    x_centroids = []
    y_centroids = []
    for contour in list_of_contours:
        temp_thresholded_cell_image = np.zeros(
            thresholded_cell_image.shape, dtype=np.uint8)
        temp_thresholded_cell_image = cv2.drawContours(
            temp_thresholded_cell_image, [contour], contourIdx=-1, color=1, thickness=-1)
        x_centroid = np.sum(temp_thresholded_cell_image*x) / \
            np.sum(temp_thresholded_cell_image)
        y_centroid = np.sum(temp_thresholded_cell_image*y) / \
            np.sum(temp_thresholded_cell_image)
...
def remove_cells_outside_circular_region(thresholded_cell_image, center, radius):
    ...
    list_of_contours, hierarchy = cv2.findContours(
        thresholded_cell_image, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)[1:]
    new_thresholded_cell_image = np.zeros(
        thresholded_cell_image.shape, dtype=np.uint8)
    ...
    for contour in list_of_contours:
        ...
        y_centroid = np.sum(temp_thresholded_cell_image*y) / \
            np.sum(temp_thresholded_cell_image)
        distance_from_circle_center = np.sqrt(
            (x_centroid-center[0])**2+(y_centroid-center[1])**2)
